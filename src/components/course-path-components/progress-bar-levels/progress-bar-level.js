...
const ProgressBarLevel = ({ level, index }) => {
    ...
    return <div className={`level_container ${showLessons ? "open" : ""}`}>
        <div className="level" onClick={() => { setShowLessons(prev => !prev) }}>
            ...
            {
                showLessons ?
                    <img src="/icons/arrows/Arrow_Up.svg"
                        className="level_lesson_arrow_width"
                        alt="" /> :
                    <img src="/icons/arrows/Arrow_Down.svg"
                        className="level_lesson_arrow_width"
                        alt="" />
            }
        </div>
        ...
        }
        {
            ...
            level.steps.map((step, index) =>
                <div key={index}
                    className={`step ${step.state === "done" ? "done_container" : step.state === "ongoing" ? "ongoing_container" : ""}`}>
                    <div className={`${step.state === ''}`} />
                    <div className={`${step.state === "done" ? "done" : step.state === "ongoing" ? "ongoing" : "pending"}`}>
                        {
                            step.state === "done" &&
                            <img src="/icons/lesson_state/correct_mark_step.svg" alt="" />
                        }
                    </div>
